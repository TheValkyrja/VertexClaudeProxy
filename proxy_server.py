...
def run_load_balance_test(num_iterations=20, test_project_ids=None):
    if test_project_ids is None:
        test_project_ids = ['project1', 'project2', 'project3']
    
    print("Testing WeightedRandomSelector:")
    test_selector = WeightedRandomSelector(test_project_ids)
    
    for i in range(num_iterations):
        selected_project = test_selector.get_project()
        print(f"Round {i+1}: Selected {selected_project}")
        print(f"Current weights: {test_selector.projects}")
        print(f"Total weight: {sum(test_selector.projects.values()):.4f}")
        print("-" * 40)

    print("\nTesting load_balance_selector function:")
    try:
...
def prepare_request(data: Dict[Any, Any], headers: Dict[str, str], project_id: str, auth_file: str) -> tuple:
    ...
    # 修改：使用选择的 project_id
    url = f"https://{region}-aiplatform.googleapis.com/v1/projects/{project_id}/locations/{region}/publishers/anthropic/models/{model}:streamRawPredict"#"http://127.0.0.1:10328/v1/messages"#
